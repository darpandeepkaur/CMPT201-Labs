...
port number).
- The server address is 127.0.0.1 (localhost) and the port number is 8000.
...
- It gets the data from standard input (the keyboard / terminal).

4. How does the client program end? How can you tell that in the code?
- The client program ends when the user enters EOF or a line shorter than 2
bytes.
*/
...
void *handle_client(void *arg) {
  ...
  while ((num_read = read(cfd, buf, BUF_SIZE)) > 0) {
    ...
    // Increment total_message_count safely
    ...
    // Print message
    printf("Msg # %d; Client ID %d: %s", msg_num, id, buf);
  }

  // Client closed connection / read error
...
int main() {
  ...
  for (;;) {
    ...
    // Allocate thread argument struct
    ...
    // Do not join; allow automatic cleanup
    pthread_detach(t);
  }

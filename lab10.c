...
- The client uses a TCP connection (AF_INET, SOCK_STREAM). It connects
to the server with connect(), then sends messages using write().
All communication uses the TCP stream protocol.

2. What data is the client sending to the server?
- The client sends 5 messages: "Hello", "Apple", "Car", "Green", "Dog".
Each message is copied into a 1024-byte buffer and the entire buffer
is written to the socket every time.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
- The thread receives a pointer to a struct acceptor_args. This struct
contains the run flag, the shared message list, and the mutex used
to protect the list. The acceptor thread needs access to those shared
values, so they are grouped together into one argument pointer.

2. How are received messages stored?
- Each client thread reads from its socket into a buffer. When a message
is received, the thread allocates a new list_node, allocates space for
the message, copies the message, and appends the node to the shared
linked list. A mutex is used to ensure thread-safe access.

3. What does `main()` do with the received messages?
- main() waits until the number of stored messages reaches
MAX_CLIENTS * NUM_MSG_PER_CLIENT. It then stops the acceptor thread,
joins it, and calls collect_all() to print and free every stored
message. Finally, it prints a summary and exits.

4. How are threads used in this sample code?
- One acceptor thread runs in the background, repeatedly calling accept()
to handle new clients. For each client, a new client-handling thread is
created. Each client thread reads messages from its socket and adds them
to the shared list. When shutting down, all client threads and the
acceptor thread are signaled to stop and then joined cleanly.
*/
...
int main() {
  ...
  while (list_handle.count < needed) {
    usleep(1000); // sleep for 1ms to avoid busy spinning too hard
  }
